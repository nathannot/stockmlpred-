...
if models == 'XGBoost (default)':
    ...
    model = LightGBMModel(lags=7, output_chunk_length=4, n_jobs=-1, random_state=42)
elif models == 'Linear Regression':
    model = LinearRegressionModel(lags=7, output_chunk_length=4, n_jobs=-1, random_state=42)
else:
